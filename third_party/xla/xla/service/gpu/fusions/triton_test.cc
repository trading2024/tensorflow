...
TEST_F(TritonFusionTest,
  ...
  auto module = ParseAndReturnVerifiedModule(R"(
    ...
    ENTRY main {
      ...
      ROOT triton_softmax = f32[125,127]{1,0} fusion(auxiliary_fusion), kind=kCustom, calls=triton_softmax_computation, backend_config={"fusion_backend_config":{"kind":"__triton","block_level_fusion_config":{"output_tile_sizes":["1","127"],"num_warps":"2"}}}
      })")
                    .value();
  ...
  auto launch_dimensions = triton_fusion->launch_dimensions();
  ASSERT_NE(launch_dimensions, std::nullopt);
...
TEST_F(TritonFusionTest,
       TritonFusionWithoutBlockLevelFusionConfig_LaunchDimensionsAreNullopt) {
#ifndef GOOGLE_CUDA
  GTEST_SKIP() << "Triton fusion only enable for CUDA devices.";
#endif
  ...
  ASSERT_NE(triton_fusion, nullptr);
  auto launch_dimensions = triton_fusion->launch_dimensions();
  EXPECT_EQ(launch_dimensions, std::nullopt);
}
